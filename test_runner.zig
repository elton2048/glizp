/// Support file for test usage. A custom test runner for a prettier
/// test result.
...
pub fn main() !void {
    ...
    try std.posix.getrandom(std.mem.asBytes(&1234));
    ...
    for (builtin.test_functions) |t| {
        ...
        const file_name = filename(t.name);
        ...
        if (env.verbose) {
            ...
            if (INCLUDE_FILE_NAME) {
                printer.status(status, "* {s} | {s} | ({d:.2}ms)\n", .{ file_name, friendly_name, ms });
            } else {
...
fn filename(name: []const u8) []const u8 {
...
pub fn panic(msg: []const u8, error_return_trace: ?*std.builtin.StackTrace, ret_addr: ?usize) noreturn {
    ...
    std.debug.panicExtra(error_return_trace, ret_addr, "{s}", .{msg});
}
